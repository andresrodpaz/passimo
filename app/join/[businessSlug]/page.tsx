...
type JoinData = {
  ...
  rewards: Array<{ id: string; name: string; description: string | null; cost: number }>
 ...
 * counter with 20 seconds of patience. So it asks for one required field,
 * shows the reward before asking for anything, and captures consent
 * explicitly rather than burying it.
 */
export default function JoinPage({ params }: { params: Promise<{ businessSlug: string }> }) {
  const { businessSlug } = use(params)
  return (
    <React.Suspense fallback={<LoadingScreen />}>
      <JoinFlow slug={businessSlug} />
    </React.Suspense>
  )
}
...
function JoinFlow({ slug }: { slug: string }) {
  const { t, locale } = useI18n()
  const searchParams = useSearchParams()
  const referralCode = searchParams.get('ref') ?? undefined

  const [data, setData] = React.useState<JoinData | null>(null)
  // The *reason* the load failed, not the sentence describing it. Translating
  // at render time keeps the fetch out of the language's way: switching locale
  // re-renders the message without re-fetching the program.
  const [loadError, setLoadError] = React.useState<'notFound' | 'loadFailed' | null>(null)
  ...
  React.useEffect(() => {
    apiGet<JoinData>(`/api/v1/public/business/${encodeURIComponent(slug)}`)
      .then(setData)
      .catch((cause) =>
        setLoadError(
          cause instanceof ApiError && cause.status === 404 ? 'notFound' : 'loadFailed'
        )
      )
  }, [slug])
    ...
    return (
      <main className="flex min-h-screen items-center justify-center p-4 text-center">
        <p className="text-sm text-muted-foreground">
          {loadError === 'notFound' ? t('join.notFound') : t('join.loadFailed')}
        </p>
      </main>
    )
  }
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div className="space-y-2.5 rounded-lg border p-3">
            <label className="flex items-start gap-2.5 text-xs leading-relaxed">
              ...
              />
              <span id="terms-text">
                {t('join.consentTerms', { business: business.name })}
              </span>
            </label>
            <label className="flex items-start gap-2.5 text-xs leading-relaxed">
              <Checkbox
